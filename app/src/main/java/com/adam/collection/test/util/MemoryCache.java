...
public class MemoryCache {
	...
	public static MemoryCache getInstance() {
		if (mInstance == null) {
			synchronized (MemoryCache.class) {
				if (mInstance == null) {
					mInstance = new MemoryCache(1, Type.LIFO);
				}
			}
		}
		return mInstance;
	 ...
	 * @return
	 */
	public static MemoryCache getInstance(int threadCount, Type type) {
		if (mInstance == null) {
			synchronized (MemoryCache.class) {
				if (mInstance == null) {
					mInstance = new MemoryCache(threadCount, type);
				}
			}
		}
		return mInstance;
	...
	private MemoryCache(int threadCount, Type type) {
	...
	private Bitmap decodeSampledBitmapFromResource(String pathName, int reqWidth, int reqHeight) {
		...
		options.inPurgeable = true; // 允许可清除,可以让java系统内存不足时先行回收部分的内存
	...
	private int calculateInSampleSize(BitmapFactory.Options options, int reqWidth, int reqHeight) {
		...
		if (width > reqWidth && height > reqHeight) { // 计算出实际宽度和目标宽度的比率
